...
module RAS(
  ...
  output [2:0]   io_out_last_stage_spec_info_ssp,
  output [2:0]   io_out_last_stage_spec_info_sctr,
  output         io_out_last_stage_spec_info_TOSW_flag,
  output [3:0]   io_out_last_stage_spec_info_TOSW_value,
  output         io_out_last_stage_spec_info_TOSR_flag,
  output [3:0]   io_out_last_stage_spec_info_TOSR_value,
  output         io_out_last_stage_spec_info_NOS_flag,
  output [3:0]   io_out_last_stage_spec_info_NOS_value,
  ...
  reg  [3:0]  s3_meta_TOSW_value;
  reg         s3_meta_TOSR_flag;
  reg  [3:0]  s3_meta_TOSR_value;
  reg         s3_meta_NOS_flag;
  reg  [3:0]  s3_meta_NOS_value;
  ...
  reg  [2:0]  redirect_next_bits_r_cfiUpdate_ssp;
  reg  [2:0]  redirect_next_bits_r_cfiUpdate_sctr;
  reg         redirect_next_bits_r_cfiUpdate_TOSW_flag;
  reg  [3:0]  redirect_next_bits_r_cfiUpdate_TOSW_value;
  reg         redirect_next_bits_r_cfiUpdate_TOSR_flag;
  reg  [3:0]  redirect_next_bits_r_cfiUpdate_TOSR_value;
  reg         redirect_next_bits_r_cfiUpdate_NOS_flag;
  reg  [3:0]  redirect_next_bits_r_cfiUpdate_NOS_value;
  reg         updateValid;
  ...
  reg  [2:0]  updateMeta_ssp;
  reg         updateMeta_TOSW_flag;
  reg  [3:0]  updateMeta_TOSW_value;
  ...
  always @(posedge clock) begin
    ...
    if (io_update_valid) begin
      ...
      updateMeta_ssp <= io_update_bits_meta[7:5];
      updateMeta_TOSW_flag <= io_update_bits_meta[4];
      updateMeta_TOSW_value <= io_update_bits_meta[3:0];
    end
  end // always @(posedge)
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        s3_top = {_RANDOM[6'h12][31:26], _RANDOM[6'h13], _RANDOM[6'h14][11:0]};
        s3_spec_new_addr = {_RANDOM[6'h14][31:12], _RANDOM[6'h15][29:0]};
        s3_pushed_in_s2 = _RANDOM[6'h15][30];
        s3_popped_in_s2 = _RANDOM[6'h15][31];
        s3_meta_ssp = _RANDOM[6'h16][2:0];
        s3_meta_sctr = _RANDOM[6'h16][5:3];
        s3_meta_TOSW_flag = _RANDOM[6'h16][6];
        s3_meta_TOSW_value = _RANDOM[6'h16][10:7];
        s3_meta_TOSR_flag = _RANDOM[6'h16][11];
        s3_meta_TOSR_value = _RANDOM[6'h16][15:12];
        s3_meta_NOS_flag = _RANDOM[6'h16][16];
        s3_meta_NOS_value = _RANDOM[6'h16][20:17];
        redirect_next_valid_last_REG = _RANDOM[6'h16][21];
        redirect_next_bits_r_level = _RANDOM[6'h17][6];
        redirect_next_bits_r_cfiUpdate_pc = {_RANDOM[6'h17][31:8], _RANDOM[6'h18][25:0]};
        redirect_next_bits_r_cfiUpdate_pd_valid = _RANDOM[6'h18][26];
        redirect_next_bits_r_cfiUpdate_pd_isRVC = _RANDOM[6'h18][27];
        redirect_next_bits_r_cfiUpdate_pd_isCall = _RANDOM[6'h18][30];
        redirect_next_bits_r_cfiUpdate_pd_isRet = _RANDOM[6'h18][31];
        redirect_next_bits_r_cfiUpdate_ssp = _RANDOM[6'h19][2:0];
        redirect_next_bits_r_cfiUpdate_sctr = _RANDOM[6'h19][5:3];
        redirect_next_bits_r_cfiUpdate_TOSW_flag = _RANDOM[6'h19][6];
        redirect_next_bits_r_cfiUpdate_TOSW_value = _RANDOM[6'h19][10:7];
        redirect_next_bits_r_cfiUpdate_TOSR_flag = _RANDOM[6'h19][11];
        redirect_next_bits_r_cfiUpdate_TOSR_value = _RANDOM[6'h19][15:12];
        redirect_next_bits_r_cfiUpdate_NOS_flag = _RANDOM[6'h19][16];
        redirect_next_bits_r_cfiUpdate_NOS_value = _RANDOM[6'h19][20:17];
        updateValid = _RANDOM[6'h25][19];
        update_r_ftb_entry_isCall = _RANDOM[6'h29][20];
        update_r_ftb_entry_isRet = _RANDOM[6'h29][21];
        update_r_ftb_entry_tailSlot_offset = _RANDOM[6'h2A][15:12];
        update_r_ftb_entry_tailSlot_valid = _RANDOM[6'h2A][17];
        update_r_cfi_idx_valid = _RANDOM[6'h2B][16];
        update_r_cfi_idx_bits = _RANDOM[6'h2B][20:17];
        update_r_jmp_taken = _RANDOM[6'h2B][25];
        updateMeta_ssp = _RANDOM[6'h37][8:6];
        updateMeta_TOSW_flag = _RANDOM[6'h37][9];
        updateMeta_TOSW_value = _RANDOM[6'h37][13:10];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        s2_pc_dup_s2_pc_seg_0_value = 26'h0;
        s2_pc_dup_s2_pc_seg_1_value = 12'h0;
  ...
  RASStack RASStack (
    ...
    .io_s3_cancel
      (io_s3_fire_2 & (s3_pushed_in_s2 != s3_push | s3_popped_in_s2 != s3_pop)
       & ~_RASStack_io_spec_near_overflow),
    ...
    .io_redirect_valid
      (redirect_next_valid_last_REG
       & (redirect_next_bits_r_cfiUpdate_TOSW_flag ^ _RASStack_io_TOSW_flag
          ^ redirect_next_bits_r_cfiUpdate_TOSW_value < _RASStack_io_TOSW_value
          | ~_RASStack_io_spec_near_overflow)),
    .io_redirect_isCall
  ...
  assign io_out_last_stage_meta =
    {252'h0, s3_meta_ssp, s3_meta_TOSW_flag, s3_meta_TOSW_value};
