...
module ICacheMSHR(
  ...
  reg  [1:0]  vSetIdx;
  reg  [1:0]  way;
  wire        io_req_ready_0 = ~valid & ~io_fencei;
  wire        io_acquire_valid_0 = valid & ~issue & ~io_fencei;
  wire        _GEN = io_req_ready_0 & io_req_valid;
  wire        _GEN_0 = io_acquire_ready & io_acquire_valid_0;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      valid <= ~io_invalid & (_GEN | ~(io_fencei & ~issue) & valid);
      flush <= ~_GEN & (io_fencei | flush);
      fencei <= ~_GEN & (io_fencei | fencei);
      issue <= _GEN_0 | ~_GEN & issue;
      if (_GEN) begin
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        vSetIdx = _RANDOM[1'h1][15:14];
        way = _RANDOM[1'h1][17:16];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        valid = 1'h0;
        flush = 1'h0;
