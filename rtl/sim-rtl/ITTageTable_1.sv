...
module ITTageTable_1(
  ...
  input  [6:0]  io_req_bits_folded_hist_hist_10_folded_hist,
  input  [5:0]  io_req_bits_folded_hist_hist_9_folded_hist,
  ...
  wire [6:0] _table_io_r_resp_data_0_tag;
  wire       _useful_can_reset_T = ~powerOnResetState & io_req_valid;
  reg  [6:0] s1_tag;
  reg        s1_valid;
  reg        s1_read_write_conflict;
  wire [6:0] _update_folded_hist_hist_10_folded_hist_T_1 =
    {io_update_ghist[6:1], io_update_ghist[0] ^ io_update_ghist[7]};
  wire [6:0] update_idx = io_update_pc[7:1] ^ _update_folded_hist_hist_10_folded_hist_T_1;
  reg        needReset;
  ...
  always @(posedge clock) begin
    if (_useful_can_reset_T)
      s1_tag <=
        io_req_bits_pc[14:8] ^ io_req_bits_folded_hist_hist_10_folded_hist
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        s1_tag = _RANDOM[/*Zero width*/ 1'b0][13:7];
        s1_valid = _RANDOM[/*Zero width*/ 1'b0][14];
        s1_read_write_conflict = _RANDOM[/*Zero width*/ 1'b0][15];
        needReset = _RANDOM[/*Zero width*/ 1'b0][16];
        resetSet = _RANDOM[/*Zero width*/ 1'b0][23:17];
        powerOnResetState = _RANDOM[/*Zero width*/ 1'b0][24];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        needReset = 1'h0;
        resetSet = 7'h0;
  ...
  FoldedSRAMTemplate_16 table_0 (
    .clock                                          (clock),
    .reset                                          (reset),
    .io_r_req_ready                                 (_table_io_r_req_ready),
    .io_r_req_valid                                 (_useful_can_reset_T),
    .io_r_req_bits_setIdx
      (io_req_bits_pc[7:1] ^ io_req_bits_folded_hist_hist_10_folded_hist),
    ...
    .io_w_req_bits_data_0_tag
      (io_update_pc[14:8] ^ _update_folded_hist_hist_10_folded_hist_T_1
       ^ {io_update_ghist[5:2], io_update_ghist[1:0] ^ io_update_ghist[7:6], 1'h0}),
    ...
    .io_w_req_bits_bitmask
      (io_update_uValid & io_update_valid
         ? 36'hFFFFFFFFF
         : io_update_valid ? 36'hFFFFFFFFE : {35'h0, useful_can_reset})
  );
  WrBypass_29 wrbypass (
    .clock              (clock),
    .reset              (reset),
