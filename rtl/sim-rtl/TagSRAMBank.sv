...
module TagSRAMBank(
  ...
  reg  [1:0]  rst_cnt;
  wire [47:0] wdata = rst_cnt[1] ? {io_write_bits_ecc, io_write_bits_tag} : 48'h0;
  wire        wen = ~(rst_cnt[1]) | io_write_valid;
  always @(posedge clock or posedge reset) begin
    if (reset)
      rst_cnt <= 2'h0;
    else if (rst_cnt[1]) begin
    end
    else
      rst_cnt <= 2'(rst_cnt + 2'h1);
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        rst_cnt = _RANDOM[/*Zero width*/ 1'b0][1:0];
      `endif // RANDOMIZE_REG_INIT
      if (reset)
        rst_cnt = 2'h0;
    end // initial
  ...
  SRAMTemplate_102 tag_array (
    ...
    .io_w_req_valid        (wen),
    .io_w_req_bits_setIdx  (rst_cnt[1] ? io_write_bits_idx : rst_cnt[0]),
    .io_w_req_bits_data_0  (wdata),
    .io_w_req_bits_data_1  (wdata),
    .io_w_req_bits_waymask (rst_cnt[1] ? io_write_bits_way_en : 2'h3)
  );
